...
class ArrTest extends TestCase
{
    ...
    {
        foreach ($this->test_arrs_snake as $test_arr) {
        ...
        foreach ($cameled as $arr) {
            ...
            foreach ($keys as $value) {
                $this->assertEquals($value, array_keys($this->proper_arrs_snake[$i])[$j]);
                $j++;
            }
            $i++;
        } //ここまでがcamel_keysのテスト
        // ここからsnake_keysのテスト
        foreach ($this->test_arrs_camel as $test_arr) {
        ...
        foreach ($snaked as $arr) {
            ...
            $keys = array_keys($arr); //配列をひとつづつキーだけ取り出す
            foreach ($keys as $value) {
                $this->assertEquals($value, array_keys($this->proper_arrs_camel[$k])[$l]);
                $l++;
            }
            $k++;
        }
